...
<div id="modal-form" class="fixed inset-0 bg-black bg-opacity-50 z-50 hidden flex items-center justify-center p-4">
    <div class="bg-gray-800 rounded-lg p-4 sm:p-6 w-full max-w-4xl mx-auto max-h-[90vh] overflow-y-auto">
        <div class="flex justify-between items-center mb-4">
            <h3 id="modal-title" class="text-xl font-bold text-white">Tambah Anggota Cabang</h3>
        ...
        <form id="anggota-form" class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <!-- Section Anggota Cabang -->
            <div class="md:col-span-2 mt-4">
                <h4 class="text-lg font-semibold text-white mb-2">Informasi Anggota</h4>
            </div>
            <div class="md:col-span-1">
                <label for="cabang_id" class="block text-white mb-1">Cabang</label>
                <select id="cabang_id" name="cabang_id" required class="w-full px-3 py-2 bg-gray-700 text-white rounded-md border border-gray-600 focus:border-blue-500 focus:outline-none">
...
<div id="view-modal" class="fixed inset-0 bg-black bg-opacity-50 z-50 hidden flex items-center justify-center p-4">
    <div class="bg-gray-800 rounded-lg p-4 sm:p-6 w-full max-w-4xl mx-auto max-h-[90vh] overflow-y-auto">
        <div class="flex justify-between items-center mb-4">
            <h3 class="text-xl font-bold text-white">Detail Anggota Cabang</h3>
...
$(document).ready(function () {
    ...
    $('#openAddModal').click(function () {
        resetForm();
        $('#modal-title').text('Tambah Anggota Cabang');
    ...
    $('#anggota-form').submit(function (e) {
        ...
        var url = id ? "{{ route('anggotacabang.update', ':id') }}".replace(':id', id) : "{{ route('anggotacabang.store') }}";
    ...
    $(document).on('click', '.view-btn', function () {
        ...
        $.get("{{ route('anggotacabang.edit', ':id') }}".replace(':id', id), function (response) {
    ...
    $(document).on('click', '.edit-btn', function () {
        ...
        $.get("{{ route('anggotacabang.edit', ':id') }}".replace(':id', id), function (response) {
            ...
            // Isi data anggota cabang
            $('#cabang_id').val(response.cabang_id);
            $('#jabatan').val(response.jabatan);
            $('#status').val(response.status);

            $('#modal-title').text('Edit Anggota Cabang');
    ...
    $(document).on('click', '.delete-btn', function () {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "{{ route('anggotacabang.destroy', ':id') }}".replace(':id', id),
                    type: 'DELETE',
