...
<div class="p-4">
    ...
    <div class="bg-gray-800 p-4 rounded-lg mb-4">
        <div class="flex justify-between items-center mb-4">
            <h2 class="text-lg font-semibold text-white">Daftar Desa</h2>
            <div>
                ...
                <a href="{{ route('desa.export') }}" class="py-1 px-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 inline-block"><i class="fas fa-file-excel h-[16px] w-[14px]"></i></a>
            </div>
        </div>
        <div class="mb-3">
            ...
            <table id="dataTable" class="w-full table-auto text-sm text-white">
                <thead>
                    <tr class="border-b border-gray-700 text-center">
                        ...
                        <th class="pb-2 text-left">Nama Desa</th>
...
<div id="modal-form" class="fixed inset-0 bg-black bg-opacity-50 z-50 hidden flex items-center justify-center p-4">
    <div class="bg-gray-800 rounded-lg p-4 sm:p-6 w-full max-w-4xl mx-auto max-h-[90vh] overflow-y-auto">
        <div class="flex justify-between items-center mb-4">
            <h3 id="modal-title" class="text-xl font-bold text-white">Tambah Desa</h3>
            <button class="text-gray-400 hover:text-white close-modal">
                ✖
            </button>
        </div>
        <form id="desa-form" class="grid grid-cols-1 md:grid-cols-2 gap-4">
            @csrf
            <input type="hidden" name="id" id="desa-id">
            <div class="md:col-span-1">
                <label for="kode" class="block text-white mb-1">Kode Desa</label>
            ...
            <div class="md:col-span-1">
                <label for="nama" class="block text-white mb-1">Nama Desa</label>
...
<div id="view-modal" class="fixed inset-0 bg-black bg-opacity-50 z-50 hidden flex items-center justify-center p-4">
    <div class="bg-gray-800 rounded-lg p-4 sm:p-6 w-full max-w-4xl mx-auto max-h-[90vh] overflow-y-auto">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="md:col-span-1">
                <p class="text-gray-400 mb-1">Kode Desa:</p>
                <p id="view-kode" class="text-white font-medium"></p>
            </div>
            <div class="md:col-span-1">
                <p class="text-gray-400 mb-1">Nama Desa:</p>
...
<script>
    $(document).ready(function () {
        ...
        // Submit Form
        $('#desa-form').submit(function (e) {
            e.preventDefault();
            var id = $('#desa-id').val();
            var url = id ? "{{ route('desa.update', ':id') }}".replace(':id', id) : "{{ route('desa.store') }}";
        ...
        $(document).on('click', '.view-btn', function () {
            ...
            $.get("{{ route('desa.edit', ':id') }}".replace(':id', id), function (response) {
        ...
        $(document).on('click', '.edit-btn', function () {
            ...
            $.get("{{ route('desa.edit', ':id') }}".replace(':id', id), function (response) {
                $('#desa-id').val(response.id);
                $('#kode').val(response.kode);
                $('#nama').val(response.nama);
                $('#kecamatan').val(response.kecamatan_id);
                $('#modal-title').text('Edit Desa');
        ...
        $(document).on('click', '.delete-btn', function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('desa.destroy', ':id') }}".replace(':id', id),
                        type: 'DELETE',
