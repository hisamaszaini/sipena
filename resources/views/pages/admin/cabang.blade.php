    ...
    <div class="bg-gray-800 p-4 rounded-lg mb-4">
        <div class="flex justify-between items-center mb-4">
            ...
            <div>
                ...
                <a href="{{ route('cabang.export') }}" class="py-1 px-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 inline-block"><i class="fas fa-file-excel h-[16px] w-[14px]"></i></a>
            </div>
        </div>
        <div class="mb-3">
            <select id="kecamatan-filter" class="p-2 bg-gray-700 text-white rounded-md w-full">
            ...
            <table id="dataTable" class="w-full table-auto text-sm text-white">
                <thead>
                    <tr class="border-b border-gray-700 text-center">
                        ...
                        <th class="pb-2 text-left">Kecamatan</th>
...
<div id="modal-form" class="fixed inset-0 bg-black bg-opacity-50 z-50 hidden flex items-center justify-center p-4">
    <div class="bg-gray-800 rounded-lg p-4 sm:p-6 w-full max-w-4xl mx-auto max-h-[90vh] overflow-y-auto">
        <div class="flex justify-between items-center mb-4">
            <h3 id="modal-title" class="text-xl font-bold text-white">Tambah Cabang</h3>
            <button class="text-gray-400 hover:text-white close-modal">
                ✖
            </button>
        </div>
        <form id="cabang-form" class="grid grid-cols-1 md:grid-cols-2 gap-4">
            @csrf
            <input type="hidden" name="id" id="cabang-id">
            <div class="md:col-span-1">
                <label for="nama" class="block text-white mb-1">Nama Cabang</label>
            ...
            <div class="md:col-span-1">
                <label for="kecamatan" class="block text-white mb-1">Kecamatan</label>
            ...
            <div class="md:col-span-1">
                <label for="daerah" class="block text-white mb-1">Daerah</label>
                <select id="daerah" name="daerah_id" required
                    class="w-full px-3 py-2 bg-gray-700 text-white rounded-md border border-gray-600 focus:border-blue-500 focus:outline-none">
                    @foreach($daerah as $dae)
                    <option value="{{ $dae->id }}">{{ $dae->nama }}</option>
                    @endforeach
                </select>
            </div>
...
<div id="view-modal" class="fixed inset-0 bg-black bg-opacity-50 z-50 hidden flex items-center justify-center p-4">
    <div class="bg-gray-800 rounded-lg p-4 sm:p-6 w-full max-w-4xl mx-auto max-h-[90vh] overflow-y-auto">
        <div class="flex justify-between items-center mb-4">
            <h3 class="text-xl font-bold text-white">Detail Cabang</h3>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="md:col-span-1">
                <p class="text-gray-400 mb-1">Nama Cabang:</p>
            ...
            <div class="md:col-span-1">
                <p class="text-gray-400 mb-1">Kecamatan:</p>
                <p id="view-kecamatan" class="text-white font-medium"></p>
            </div>
            <div class="md:col-span-2">
    ...
    $(document).ready(function () {
        var table = $('#dataTable').DataTable({
            ...
            ajax: {
                ...
                data: function (d) {
                    d.kecamatan_id = $('#kecamatan-filter').val();
                }
            },
            columns: [
                {
        ...
        $('#cabang-form').submit(function (e) {
            e.preventDefault();
            var id = $('#cabang-id').val();
            var url = id ? "{{ route('cabang.update', ':id') }}".replace(':id', id) : "{{ route('cabang.store') }}";
        ...
        $(document).on('click', '.view-btn', function () {
            ...
            $.get("{{ route('cabang.edit', ':id') }}".replace(':id', id), function (response) {
                ...
                $('#view-kecamatan').text(response.kecamatan.nama);
        ...
        $(document).on('click', '.edit-btn', function () {
            ...
            $.get("{{ route('cabang.edit', ':id') }}".replace(':id', id), function (response) {
                $('#cabang-id').val(response.id);
                $('#nama').val(response.nama);
                $('#nomor_sk').val(response.nomor_sk);
                $('#nama_pimpinan').val(response.nama_pimpinan);
                $('#no_telp').val(response.no_telp);
                $('#daerah').val(response.daerah.id);
                $('#kecamatan').val(response.kecamatan.id);
                $('#alamat').val(response.alamat);
                $('#modal-title').val('Edit Cabang');
