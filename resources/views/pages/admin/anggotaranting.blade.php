...
<div class="p-4">
    ...
    <div class="bg-gray-800 p-4 rounded-lg mb-4">
        <div class="flex justify-between items-center mb-4">
            <h2 class="text-lg font-semibold text-white">Daftar Anggota Ranting</h2>
            <div>
                ...
                <a href="{{ route('anggotaranting.export') }}" class="py-1 px-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 inline-block"><i class="fas fa-file-excel h-[16px] w-[14px]"></i></a>
            </div>
        </div>
            ...
            <table id="dataTable" class="w-full table-auto text-sm text-white">
                <thead>
                    <tr class="border-b border-gray-700 text-center">
                        ...
                        <th class="pb-2 text-left">Ranting</th>
...
<div id="modal-form" class="fixed inset-0 bg-black bg-opacity-50 z-50 hidden flex items-center justify-center p-4">
    <div class="bg-gray-800 rounded-lg p-4 sm:p-6 w-full max-w-4xl mx-auto max-h-[90vh] overflow-y-auto">
        <div class="flex justify-between items-center mb-4">
            <h3 id="modal-title" class="text-xl font-bold text-white">Tambah Anggota Ranting</h3>
        ...
        <form id="anggota-form" class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <!-- Section Anggota Ranting -->
            <div class="md:col-span-2 mt-4">
                <h4 class="text-lg font-semibold text-white mb-2">Informasi Anggota</h4>
            </div>
            <div class="md:col-span-1">
                <label for="ranting_id" class="block text-white mb-1">Ranting</label>
                <select id="ranting_id" name="ranting_id" required class="w-full px-3 py-2 bg-gray-700 text-white rounded-md border border-gray-600 focus:border-blue-500 focus:outline-none">
                        <option value="">Pilih Ranting</option>
                    @foreach($ranting as $cb)
...
<div id="view-modal" class="fixed inset-0 bg-black bg-opacity-50 z-50 hidden flex items-center justify-center p-4">
    <div class="bg-gray-800 rounded-lg p-4 sm:p-6 w-full max-w-4xl mx-auto max-h-[90vh] overflow-y-auto">
        <div class="flex justify-between items-center mb-4">
            <h3 class="text-xl font-bold text-white">Detail Anggota Ranting</h3>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <!-- Informasi Anggota -->
            <div class="md:col-span-1">
                <p class="text-gray-400 mb-1">Ranting:</p>
                <p id="view-ranting" class="text-white font-medium"></p>
            </div>
            <div class="md:col-span-1">
...
$(document).ready(function () {
    ...
    $('#openAddModal').click(function () {
        resetForm();
        $('#modal-title').text('Tambah Anggota Ranting');
    ...
    $('#anggota-form').submit(function (e) {
        ...
        var url = id ? "{{ route('anggotaranting.update', ':id') }}".replace(':id', id) : "{{ route('anggotaranting.store') }}";
    ...
    $(document).on('click', '.view-btn', function () {
        ...
        $.get("{{ route('anggotaranting.edit', ':id') }}".replace(':id', id), function (response) {
            $('#view-nik').text(response.biodata.nik);
            $('#view-nama').text(response.biodata.nama);
            $('#view-ranting').text(response.ranting.nama);
    ...
    $(document).on('click', '.edit-btn', function () {
        ...
        $.get("{{ route('anggotaranting.edit', ':id') }}".replace(':id', id), function (response) {
            ...
            // Isi data anggota ranting
            $('#ranting_id').val(response.ranting_id);
            $('#jabatan').val(response.jabatan);
            $('#status').val(response.status);

            $('#modal-title').text('Edit Anggota Ranting');
    ...
    $(document).on('click', '.delete-btn', function () {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "{{ route('anggotaranting.destroy', ':id') }}".replace(':id', id),
                    type: 'DELETE',
